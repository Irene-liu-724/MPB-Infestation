...
one_bdp <- function(pop_size, birth_rate, death_rate, tau, capacity){
  ...
  for (i in seq_len(tau)){
    ...
    birth_num <- birth_rate * pop_size * (1 - pop_size/capacity)
...
birth_death_process <- function(dat, birth_rate, death_rate, tau, capacity, incell_rep){
  pi <- dat$density
  n <- length(pi)
  pop_size <- pi * capacity
  pop_matrix <- matrix(rep(pop_size, incell_rep), ncol = n, byrow = TRUE)
  pop_temp <- apply(pop_matrix, 1, one_bdp, birth_rate = birth_rate, death_rate = death_rate,
                    tau = tau, capacity = capacity)
